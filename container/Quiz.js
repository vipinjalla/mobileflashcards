...
import {
  setLocalNotification as setLocalNotificationForTomorrow,
...
export class Quiz extends React.Component {
  ...
  componentDidMount() {
    clearLocalNotification().then(setLocalNotificationForTomorrow());
  }
  ...
  toggleView = () => {
    this.setState(state => {
      return {
        view:
          state.view === QUESTIONS_VIEWS.ANSWER
            ? QUESTIONS_VIEWS.QUESTION
            : QUESTIONS_VIEWS.ANSWER,
      };
    });
  };
  ...
  nextQuestion = () => {
    this.setState((state, props) => {
      ...
      return nextQuestionIndex === questions.length
        ? { showScore: true }
        : { questionIndex: nextQuestionIndex, view: QUESTIONS_VIEWS.QUESTION };
    });
  };
  ...
  render() {
    ...
    const percentage = (this.score * 100) / questions.length;

    return showScore ? (
      <View style={styles.container}>
        <Text style={styles.question}>Your Score</Text>
        <Text style={styles.question}>{`${percentage}%`}</Text>
      </View>
    ) : (
      <View style={styles.container}>
        <Text>{`Remaining questions: ${questions.length -
          questionIndex -
          1}`}</Text>
